...
public class Fortos1 {
	public static void main(String args[]){//Fortos gia to dep 2
		File f1;
		int from_dep=1;
		f1=new File("Fortos2.txt");
		int query=0;
		int account=1;
		int action=0;
		int account_to=0;
		...
		try{
			...
			for(int i=0;i<300000;i++){
				if(i<120000) {
					...
					if(account<=90) account=random.nextInt(500)+500+1;	//Me pithanotita 90% sto idio  ,10% sto allo
					else account=random.nextInt(500)+1;				//Allios tha paie sto dep 2
				}else if (i>=120000 && i<240000){
					query=2;											//Omoia gia query 2,prosthesi-afairesi xrimaton
					account=random.nextInt(100)+1;		
					if(account<=90) account=random.nextInt(500)+500+1;			//Pithanotita 90% ,sto idio
					else account=random.nextInt(500)+1;				//Pithanotita 10% na einai se allo dep dil apo 500 kai pano sto arxeio customers
